(function () {
  ...
  async function pushAllDataToSupabase(skipPin) {
    if (!window.syncEnabled || !window.currentUser) return; try { const uid = window.currentUser.id; const prev = getLocalOwnerUid(); if (prev && prev !== uid) { handleOwnerMismatch(uid); return; } } catch { } cloudSyncStart(); try {
      const pinSkip = !!skipPin || !!window.__pinInternalWrite;
  ...
  async function pullDataFromSupabase() {
    if (!window.syncEnabled || !window.currentUser) return; cloudSyncStart(); try {
      showToast && showToast('Loading Data...', 'info', null, null, 8000); const uid = window.currentUser.id; const fetch = (table, cols) => supabase.from(table).select(cols).eq('user_id', uid);
  ...
  async function loadPrefetchedDataFromSW() {
    try {
      ...
      if (products.length) {
        const alive = products.filter(p => p && p.deleted !== true);
        const cloudProducts = alive.map(p => ({ id: `P-${(p.name || '').toLowerCase().replace(/[^a-z0-9]+/g, '_')}`, name: p.name, category: p.category, cost: parseFloat(p.cost), price: parseFloat(p.price), hasStock: (p.has_stock === false) ? false : true, saleUnit: p.sale_unit || '', purchaseUnit: p.purchase_unit || '', unitsPerPurchase: (p.units_per_purchase != null ? Number(p.units_per_purchase) : null), tags: (typeof p.tags === 'string' && p.tags ? p.tags.split(',').map(t => t.trim()).filter(Boolean) : []) }));
        state.products = deduplicateByContent([...(state.products || []), ...cloudProducts], 'products');
      }
      const sales = (salesRec && salesRec.data) || [];
      if (sales.length) {
        const alive = sales.filter(s => s && s.deleted !== true);
        const cloudSales = alive.map(s => ({ id: `S-${s.timestamp}`, date: s.date, time: s.time, timestamp: s.timestamp, productName: s.product_name, customer: s.customer, quantity: s.quantity, costPerUnit: parseFloat(s.cost_per_unit), pricePerUnit: parseFloat(s.price_per_unit), totalCost: parseFloat(s.total_cost), totalPrice: parseFloat(s.total_price), profit: parseFloat(s.profit), payment: s.payment, status: s.status || 'paid', category: s.category || '', hasStock: (s.has_stock === false ? false : true), tags: (typeof s.tags === 'string' && s.tags ? s.tags.split(',').map(t => t.trim()).filter(Boolean) : []) }));
        state.sales = deduplicateByContent([...(state.sales || []), ...cloudSales], 'sales');
      }
      const categories = (categoriesRec && categoriesRec.data) || [];
      if (categories.length) {
        const alive = categories.filter(c => c && c.deleted !== true);
        const cloudCategories = alive.map(c => (c.name || '').trim()).filter(Boolean);
        state.categories = Array.from(new Set([...(state.categories || []), ...cloudCategories]));
        state.categoryKinds = state.categoryKinds || {};
        alive.forEach(c => { const nm = (c.name || '').trim(); if (nm && c.kind) state.categoryKinds[nm] = c.kind; });
      }
      const expenses = (expensesRec && expensesRec.data) || [];
      if (expenses.length) {
        const alive = expenses.filter(e => e && e.deleted !== true);
        const cloudExpenses = alive.map(e => ({ id: e.id || `E-${e.timestamp}`, date: e.date, time: e.time, timestamp: e.timestamp, description: e.description, category: e.category, amount: parseFloat(e.amount), payment: e.payment, comment: e.comment, tags: (typeof e.tags === 'string' && e.tags ? e.tags.split(',').map(t => t.trim()).filter(Boolean) : []) }));
        state.expenses = deduplicateByContent([...(state.expenses || []), ...cloudExpenses], 'expenses');
      }
      const income = (incomeRec && incomeRec.data) || [];
      if (income.length) {
        const alive = income.filter(i => i && i.deleted !== true);
        const cloudIncome = alive.map(i => ({ id: `I-${i.timestamp}`, date: i.date, time: i.time, timestamp: i.timestamp, source: i.source, amount: parseFloat(i.amount), payment: i.payment, comment: i.comment, asCapital: i.as_capital === true, includeTithing: i.include_in_tithing !== false }));
        state.income = deduplicateByContent([...(state.income || []), ...cloudIncome], 'income');
      }
      const customers = (customersRec && customersRec.data) || [];
      if (customers.length) {
        const alive = customers.filter(c => c && c.deleted !== true);
        const cloudCustomers = alive.map(c => ({ name: c.name, email: c.email || '', phone: c.phone || '', address: c.address || '', totalPurchases: c.total_purchases || 0, tags: (typeof c.tags === 'string' && c.tags ? c.tags.split(',').map(t => t.trim()).filter(Boolean) : []) }));
        state.customers = deduplicateByContent([...(state.customers || []), ...cloudCustomers], 'customers');
      }
      const invoices = (invoicesRec && invoicesRec.data) || [];
      if (invoices.length) {
        const alive = invoices.filter(inv => inv && inv.deleted !== true);
        state.invoices = deduplicateByContent([...(state.invoices || []), ...alive], 'invoices');
      }
      const receipts = (receiptsRec && receiptsRec.data) || [];
      if (receipts.length) {
        const alive = receipts.filter(r => r && r.deleted !== true);
        state.receipts = deduplicateByContent([...(state.receipts || []), ...alive], 'receipts');
      }
      const inventory = (inventoryRec && inventoryRec.data) || [];
      if (inventory.length) {
        inventory.forEach(row => { if (row && row.deleted === true) return; const name = row.product_name; const inv = state.inventory[name] || { stock: 0, minAlert: 5 }; inv.stock = row.stock || 0; inv.minAlert = row.min_alert || inv.minAlert; state.inventory[name] = inv; });
      }
      const notes = (notesRec && notesRec.data) || [];
      if (notes.length) {
        const alive = notes.filter(n => n && n.deleted !== true);
        const cloudNotes = alive.map(n => ({ title: n.title, content: n.content, date: n.date, time: n.time, timestamp: n.timestamp, tags: (typeof n.tags === 'string' && n.tags ? n.tags.split(',').map(t => t.trim()).filter(Boolean) : (Array.isArray(n.tags) ? n.tags : [])) }));
        state.notes = deduplicateByContent([...(state.notes || []), ...cloudNotes], 'notes');
      }
      const assets = (assetsRec && assetsRec.data) || [];
      if (assets.length) {
        const alive = assets.filter(a => a && a.deleted !== true);
        const cloudAssets = alive.map(a => ({
          name: a.name,
      ...
      if (maintenance.length) {
        const aliveMaint = maintenance.filter(m => m && m.deleted !== true);
        const cloudMaint = aliveMaint.map(m => ({ assetName: m.asset_name, amount: parseFloat(m.amount) || 0, date: m.date, time: m.time, timestamp: m.timestamp, description: m.description || '', moneySource: m.money_source || 'business' }));
      ...
      if (transactions.length) {
        const aliveTx = transactions.filter(t => t && t.deleted !== true);
        const cloudTx = aliveTx.map(t => ({ channel: t.channel, customerName: t.customer_name, type: t.type, amount: parseFloat(t.amount), date: t.date, time: t.time, timestamp: t.timestamp, tags: (typeof t.tags === 'string' && t.tags ? t.tags.split(',').map(x => x.trim()).filter(Boolean) : []) }));
        state.transactions = deduplicateByContent([...(state.transactions || []), ...cloudTx], 'transactions');
      }
      const unpaid = (unpaidRec && unpaidRec.data) || [];
      if (unpaid.length) {
        const aliveUnpaid = unpaid.filter(u => u && u.deleted !== true);
        const cloudUnpaid = aliveUnpaid.map(u => ({ name: u.name, type: u.type, amount: parseFloat(u.amount), date: u.date, time: u.time, timestamp: u.timestamp, paid: !!u.paid, tags: (typeof u.tags === 'string' && u.tags ? u.tags.split(',').map(t => t.trim()).filter(Boolean) : []) }));
      ...
      if (tithingCloud.length) {
        const aliveTithing = tithingCloud.filter(r => r && r.deleted !== true);
        state.tithingRecords = state.tithingRecords || {};
        aliveTithing.forEach(r => { const mk = String(r.month_key); state.tithingRecords[mk] = { monthKey: mk, base: r.base || 0, due: r.due || 0, paid: r.paid || 0, history: r.history || [], updated_at: r.updated_at || new Date().toISOString() }; });
      }
      const tagsCloud = (tagsRec && tagsRec.data) || [];
      if (tagsCloud.length) {
        const aliveTags = tagsCloud.filter(r => r && r.deleted !== true);
        const names = aliveTags.map(r => String(r.tag_name || '').toLowerCase()).filter(Boolean);
        const unique = Array.from(new Set([...(state.tags || []), ...names]));
        state.tags = unique;
        state.tagColors = state.tagColors || {};
        aliveTags.forEach(r => { const n = String(r.tag_name || '').toLowerCase(); if (!n) return; const col = r.color || '#999999'; if (!state.tagColors[n]) state.tagColors[n] = col; });
      }
      try { saveData && saveData(); render && render(); } catch { }
    } catch { }
  }
